...
int registration_handler(tumbler_state_t state, void *socket, uint8_t *data) {
  ...
  RLC_TRY {
    ...
    for(int i=0; i<BITS_STATISTIC_PARAM; i++)
    {
      ...
      lhtlp_puzzle_new(lhtlp_ranges->Ds[i]); // The components of lhtlp_ranges still need to be allocated memory, deferent from polyfunc.
    ...
    + BITS_THRESHOLD_PARAM * BYTES_MODULUS_RSA_POWER + ((BITS_THRESHOLD_PARAM - 1) * BYTES_ECC_ORDER) + BYTES_THRESHOLD_LEN_PARAM;
...
int payment_presig_handler(tumbler_state_t state, void *socket, uint8_t *data) {
  ...
  RLC_TRY {
    ...
    if (cp_ecdsa_ver(state->sigma_ta->r, state->sigma_ta->s, tx, sizeof(tx), 0, state->tumbler_alice_ec_pk->pk) != 1) {
      RLC_THROW(ERR_CAUGHT);
    }
...
int main(void)
{
  ...
  RLC_TRY {
    ...
    ec_curve_get_ord(q);    
    bn_rand_mod(state->tumbler_ec_sk2->sk, q);
