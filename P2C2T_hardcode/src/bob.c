...
int promise_init(bob_state_t state, void *socket) {
  ...
  RLC_TRY {
    ...
    const unsigned msg_data_length = (2 * RLC_G1_SIZE_COMPRESSED) + RLC_BN_SIZE + ((2 * BITS_STATISTIC_PARAM + 1) * BYTES_MODULUS_RSA) + ((2 * BITS_STATISTIC_PARAM ) * BYTES_MODULUS_RSA_2) + (3 * RLC_CL_CIPHERTEXT_SIZE) 
      + (BITS_STATISTIC_PARAM * RLC_EC_SIZE_COMPRESSED) + (BITS_STATISTIC_PARAM * BYTES_INTERVAL_EXT_PARAM_PUB) + (BITS_STATISTIC_PARAM * BYTES_RANGE_PI_W) + BYTES_INTERVAL_PARAM_PUB + BITS_THRESHOLD_PARAM * BYTES_MODULUS_RSA_POWER + ((BITS_THRESHOLD_PARAM - 1) * BYTES_ECC_ORDER) + BYTES_THRESHOLD_LEN_PARAM + RLC_EC_SIZE_COMPRESSED;
    ...
    ec_write_bin(promise_init_msg->data + (2 * RLC_G1_SIZE_COMPRESSED) + RLC_BN_SIZE + ((2 * BITS_STATISTIC_PARAM + 1) * BYTES_MODULUS_RSA) + ((2 * BITS_STATISTIC_PARAM ) * BYTES_MODULUS_RSA_2) + (3 * RLC_CL_CIPHERTEXT_SIZE) 
      + (BITS_STATISTIC_PARAM * RLC_EC_SIZE_COMPRESSED) + (BITS_STATISTIC_PARAM * BYTES_INTERVAL_EXT_PARAM_PUB) + (BITS_STATISTIC_PARAM * BYTES_RANGE_PI_W) + BYTES_INTERVAL_PARAM_PUB + BITS_THRESHOLD_PARAM * BYTES_MODULUS_RSA_POWER
       + ((BITS_THRESHOLD_PARAM - 1) * BYTES_ECC_ORDER) + BYTES_THRESHOLD_LEN_PARAM, RLC_EC_SIZE_COMPRESSED, state->bob_ec_pk->pk, 1);      
...
int promise_com_handler(bob_state_t state, void *socket, uint8_t *data) {
  ...
  RLC_TRY {
    ...
    char *k_bob_tumbler_str = "16165152011112032190270112640967880756908496637508567332251425938706073010070";
    bn_read_str(state->rand_for_tumbler, k_bob_tumbler_str, strlen(k_bob_tumbler_str), 10);
...
int promise_decom_handler(bob_state_t state, void *socket, uint8_t *data) {
  ...
  RLC_TRY {
    ...
    printf("**pk_2 of bob and tumbler:\n");
...
int puzzle_solution_share_handler(bob_state_t state, void *socet, uint8_t *data) {
  ...
  RLC_TRY {
    ...
    // printf("wit of 2AS in bob.c:\n");
    // bn_print(alpha_inverse);
