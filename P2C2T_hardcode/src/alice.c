 ...
 * -----------------------
 * Ps(alice) and Ph(tumbler) run ΠTA2L.Solving to produce two-party adaptor signature (r, s).
...
int registration_vtd_handler(alice_state_t state, void *socket, uint8_t *data) {
  ...
  RLC_TRY {
    ...
    ec_read_bin(state->tumbler_ec_pk->pk, data + RLC_BN_SIZE + ((2 * BITS_STATISTIC_PARAM + 4) * BYTES_MODULUS_RSA) + ((2 * BITS_STATISTIC_PARAM ) * BYTES_MODULUS_RSA_2) 
    + (BITS_STATISTIC_PARAM * RLC_EC_SIZE_COMPRESSED) + (BITS_STATISTIC_PARAM * BYTES_INTERVAL_EXT_PARAM_PUB) + (BITS_STATISTIC_PARAM * BYTES_RANGE_PI_W) + BYTES_INTERVAL_PARAM_PUB 
...
int payment_init(alice_state_t state, void *socket) {
  ...
  RLC_TRY {
    ...
    char *k_alice_str = "40496093740717635224585041572336182270872691767602412395978570582592642874867";
    bn_read_str(state->rand_for_t, k_alice_str, strlen(k_alice_str), 10);
...
int payment_ctx_handler(alice_state_t state, void *socket, uint8_t *data) {
  ...
  RLC_TRY {
    ...
    printf("**pk_1 between alice and tumbler:\n");
    ec_print(state->alice_tumbler_ec_pk->pk);
