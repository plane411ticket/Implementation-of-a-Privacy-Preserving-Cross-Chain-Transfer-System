...
int cp_ecdsa_sig_for_bitcoin(bn_t r, bn_t s, const char * e_str, const char * k_str, const char * d_str) {
	bn_t n, k, x, e, d, s2;
	ec_t p;
	// uint8_t h[RLC_MD_LEN];
	int result = RLC_OK;
	...
	RLC_TRY {
		...
		ec_curve_get_ord(n);
		bn_read_str(e, e_str, strlen(e_str), 10);
		bn_read_str(k, k_str, strlen(k_str), 10);
		bn_read_str(d, d_str, strlen(d_str), 10);
...
int cp_ecdsa_ver_for_2as(const bn_t r, const bn_t s, const char * e_str, const ec_t q) {
	bn_t n, k, e, v;
	ec_t p;
	// uint8_t h[RLC_MD_LEN];
	int cmp, result = 0;
